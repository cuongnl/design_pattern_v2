...
import constant.AnnotationConst;
...
import support.scanner.TypeFilter;
 ...
 * @see annotation.Configuration 这个作为扫描的入口配置类
 * @see annotation.Component 这个作为组件类
 * @see context.AnnotationApplicationContext 注解应用上下文
 * @since 0.1.11
 */
@ThreadSafe
public class ClassPathAnnotationBeanDefinitionScanner implements AnnotationBeanDefinitionScanner {
     ...
     * （1）获取该包下所有的类名称
     * （2）通过类名称加载为类信息
     * （3）添加 {@link support.scanner.TypeFilter} 过滤信息
     * （4）构建 {@link AnnotationBeanDefinition} 定义对象
     *
     * @param scannerContext 包名称数组
     * @return 结果列表
     * @see model.AnnotationBeanDefinition 对象定义
     * @since 0.1.11
     */
    @Override
     ...
     * 如果需要自动发现，可以将 {@link Configuration} 相关的处理抽象为和 {@link annotation.Component}
     * 类似的处理逻辑。
     *
